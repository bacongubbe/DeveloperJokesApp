...
class JokeService(@Autowired private val repo: JokeRepoDAO) {
  fun getRandomJoke() : JokeDTO = repo.getRandomJoke()?.toDTO()
    ?: throw ResponseStatusException(HttpStatusCode.valueOf(404), "No Jokes in the database")
  fun getRandomJoke(language : String) : JokeDTO = repo.getRandomJoke(language)?.toDTO()
    ?: throw ResponseStatusException(HttpStatusCode.valueOf(404), "No Jokes for that language")
  fun getSpecificJoke(id : String) : JokeDTO = repo.getJoke(id)?.toDTO()
    ?: throw ResponseStatusException(HttpStatusCode.valueOf(404), "No Joke with that ID")
